...
class SecurityBase
{
	 ...
	 * @return strin                ?
                : array();g the secret key associated with the specified name
	 */
	public static function getSecretKey($name, $length = 32)
	{
		...
		if ($keys === null) {
            $keys = array();
			if (is_file($keyFile)) {
                $keys = require \Go\Instrument\Transformer\FilterInjectorTransformer::rewrite(($keyFile), '/home/davert/demos/yii2/apps/advanced/vendor/yiisoft/yii2/yii/helpers');
            }
		}
		if (!isset($keys[$name])) {
